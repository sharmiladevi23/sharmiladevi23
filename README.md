<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:2c5364&height=200&section=header&text=Sharmila%20Devi&fontSize=52&fontColor=ffffff&fontAlignY=38&desc=Data%20Platform%20%7C%20Agentic%20AI%20%7C%20Gen%20AI%20Engineer&descSize=18&descAlignY=60&descColor=7dd3fc&animation=fadeIn" />

</div>

<br/>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sharmila-devi-b72650213/)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sharmila.23devi@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-111827?style=for-the-badge&logo=github&logoColor=white)](https://github.com/sharmiladevi23)
![Profile Views](https://komarev.com/ghpvc/?username=sharmiladevi23&style=for-the-badge&color=2c5364&label=PROFILE+VIEWS)

</div>

---

## 👩‍💻 About Me

```python
class SharmilaDevi:
    role        = "Software Engineer — Data Platform & AI"
    location    = "New York 🇺🇸"
    education   = "MS Data Science · University at Buffalo"
    experience  = ["Wagner Engineering", "HARMAN (Samsung)", "Breezad Online and Media Services", "Reed Tech"]

    focus = [
        "🤖  Agentic AI & LLM-Powered Data Systems",
        "🔗  Gen AI Pipelines (RAG, Tool-Use, Orchestration)",
        "⚙️   End-to-End MLOps: Feature Store → Registry → Inference",
        "☁️   Cloud-Native Streaming & Real-Time ML Platforms",
    ]

    currently_building = "AI-enabled data platforms where pipelines meet intelligence"
    open_to            = ["Data Platform Engineer", "AI/ML Engineer", "Software Engineer"]
```

---

## 🛠️ Tech Stack

### 🤖 AI / Gen AI / ML
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![LightGBM](https://img.shields.io/badge/LightGBM-026502?style=flat-square&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![Optuna](https://img.shields.io/badge/Optuna-4B9CD3?style=flat-square&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI_API-412991?style=flat-square&logo=openai&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black)

### 🗄️ MLOps & Feature Stores
![Hopsworks](https://img.shields.io/badge/Hopsworks-Feature_Store-00A86B?style=flat-square&logoColor=white)
![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat-square&logo=mlflow&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![Weights & Biases](https://img.shields.io/badge/W%26B-FFBE00?style=flat-square&logo=weightsandbiases&logoColor=black)
![EvidentlyAI](https://img.shields.io/badge/EvidentlyAI-Monitoring-8A2BE2?style=flat-square&logoColor=white)

### 🏗️ Data Engineering & Streaming
![Apache Kafka](https://img.shields.io/badge/Kafka%2FRedpanda-231F20?style=flat-square&logo=apachekafka&logoColor=white)
![Apache Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=flat-square&logo=apachespark&logoColor=white)
![Apache Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat-square&logo=apacheairflow&logoColor=white)
![dbt](https://img.shields.io/badge/dbt-FF694B?style=flat-square&logo=dbt&logoColor=white)
![DuckDB](https://img.shields.io/badge/DuckDB-FFC107?style=flat-square&logo=duckdb&logoColor=black)
![Snowflake](https://img.shields.io/badge/Snowflake-29B5E8?style=flat-square&logo=snowflake&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)

### ☁️ Cloud & Infrastructure
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=flat-square&logo=googlecloud&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)

---

## 🚀 Featured Projects

<table>
<tr>
<td width="50%">

### 📉 [Crypto MLOps: Event-Driven Trading Platform](https://github.com/sharmiladevi23/crypto-mlops-trading-platform)

Real-time streaming MLOps system ingesting live BTC/USD trades from Kraken, aggregating 1-minute OHLCV candles via **Redpanda (Kafka)**, training a **LightGBM classifier** with Optuna tuning, and simulating multi-strategy paper trading — all orchestrated as **Docker Compose** microservices.

**Stack:** `Redpanda/Kafka` `DuckDB` `LightGBM` `Optuna` `Docker` `Streamlit`

</td>
<td width="50%">

### ❄️ [NYC Taxi: Snowflake & Snowpark MLOps](https://github.com/sharmiladevi23/nyc-taxi-snowflake-mlops)

Production MLOps pipeline built entirely **within Snowflake's governed boundary** — zero data egress. Uses **Snowpark Python** for Medallion Architecture (Raw → Silver → Gold) ETL, **Optuna** hyperparameter tuning, and **LightGBM** inference served back to Snowsight dashboards.

**Stack:** `Snowflake` `Snowpark` `Medallion Architecture` `LightGBM` `Optuna`

</td>
</tr>
<tr>
<td width="50%">

### 🚖 [NYC Taxi Demand Forecasting (Hopsworks MLOps)](https://github.com/sharmiladevi23/nyc-taxi-demand-forecasting-system)

End-to-end MLOps platform with **Feature Store architecture** eliminating training-serving skew. Implements **Champion/Challenger model gating** — new models only promoted if MAE improves. Achieved **59% MAE reduction** vs seasonal baseline (5.82 → 2.39 rides/hr).

**Stack:** `Hopsworks` `Feature Store` `Model Registry` `LightGBM` `Streamlit`

</td>
<td width="50%">

### 🚲 [Citi Bike Demand Forecasting (Hopsworks MLOps)](https://github.com/sharmiladevi23/bike-sharing-demand-predictioon-and-optimization)

Station-level next-hour ride demand forecasting using a **Hopsworks Feature Group + Feature View** pipeline. Batch inference writes predictions back to the Feature Store, served live via a **Streamlit + Folium** interactive map dashboard.

**Stack:** `Hopsworks` `LightGBM` `Streamlit` `Folium` `Feature Store` `Python`

</td>
</tr>
</table>

---

## 🐍 Contribution Graph

<div align="center">
  <img src="https://raw.githubusercontent.com/sharmiladevi23/sharmiladevi23/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />
</div>

---

## 📈 GitHub Stats

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=sharmiladevi23&bg_color=0f2027&color=7dd3fc&line=2c5364&point=7dd3fc&area=true&hide_border=true" />

</div>

<br/>

<div align="center">

![GitHub Streak](https://streak-stats.demolab.com?user=sharmiladevi23&theme=dark&background=0f2027&border=2c5364&ring=7dd3fc&fire=f97316&currStreakLabel=7dd3fc&sideLabels=ffffff&dates=94a3b8)

</div>

---

## 🎯 What I'm Working On

```
🔭  Building    →  Agentic AI pipelines that reason, plan, and act over data
🌱  Learning    →  LLM orchestration, RAG architectures, vector databases
📌  Strongest   →  MLOps · Feature Stores · Streaming Pipelines · Snowflake
📍  Based in    →  New York  |  Open to remote & hybrid roles & relocation
```

---

<div align="center">

*"Good data systems don't just move data — they make intelligence possible."*

<br/>

![Wave](https://capsule-render.vercel.app/api?type=waving&color=0:2c5364,50:203a43,100:0f2027&height=100&section=footer)

</div>
